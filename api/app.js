(function(){
  ...
  LICSYSTEM.captacao = {
    ...
    /** Dataset IBGE embutido em /municipios.json (fallback sem API serverless). */
    ...
    /** Tenta /api/municipios; se falhar, busca no JSON local. */
    fetchMunicipios: function (q) {
      ...
      return fetch("/api/municipios?q=" + encodeURIComponent(term))
        ...
        .catch(function () {
          return LICSYSTEM.captacao.searchMunicipiosLocal(term);
        });
    },
    ...
    suggestMunicipios: function (q) {
      ...
      LICSYSTEM.captacao
        ...
        .catch(function () {
          ...
          LICSYSTEM.captacao.clearQuickPick();
          box.innerHTML =
            '<div class="small muted" style="padding:10px 12px">Não foi possível carregar a lista de municípios. Recarregue a página (precisa de <code>/municipios.json</code> via <code>npm run dev</code>).</div>';
          box.hidden = false;
        });
    },
