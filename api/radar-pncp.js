...
async function handler(req, res) {
  ...
  try {
    ...
    var querLeilao = leiloesFlag || textoLeilao;

    /*
     * Sempre consulta Pregão Eletrônico (6) para palavras como "ar condicionado".
     * "Incluir leilões" SOMA modalidades 13 (e opcionalmente 1) — não substitui o pregão.
     * Antes, marcar o checkbox buscava SÓ leilão e escondia pregões normais.
     */
    var modalidades = [6];
    if (querLeilao) {
      modalidades = [6, 13];
      if (String(opts.paginas || "") === "ampliar") {
        modalidades = [6, 13, 1];
      }
    }
    var uf = opts.uf || "";
